...
def test_record(record):
    ...
    # logging.info('导入商品包装数据: %s' % item_code)
    print '导入商品包装数据: %s' % item_code
    if len(item_code) == 9 and item_code.startswith('100'):
        item_code = item_code[3:]

    try:
        frappe.get_doc('UOM Conversion Detail', {'uom': record.find('PackageID').text})
    except frappe.DoesNotExistError:
        # logging('找不到一致field: PackageID=%s和MerchID=%s'
    ...
    try:
        ...
        if item.mini_uom != int(record.find('PackageID').text) & float(record.find('MerchID').text) == 1.0:
            print
            'Item中mini_uom字段未更新'
    except frappe.DoesNotExistError:
